...
proof.
  ...
  + seq 27: ( valid trace___addstate_array_avx2 /\ 0<=_offset /\ 0<=_lEN /\ _lEN <= 200 /\
    ...
    + auto.
      ...
      auto. rewrite /valid /is_init /= => &m /> *. split; smt(all_cat).
    seq 27: ( valid trace___addstate_array_avx2 /\ 0<=_offset /\ 0<=_lEN /\ _lEN <= 200 /\
  ...
  + sp. 
    ...
    + auto .
      ...
      + auto .
        ...
        move => /> *. split.  rewrite !all_cat /=. smt(). smt(). 
      auto.  rewrite /valid /is_init /= => &m /> *.  smt(all_cat).
    auto.  rewrite /valid /is_init /= => &m /> *.  smt(all_cat).
    ...
    + if .
      + while(valid trace___squeeze_array_avx2 /\ lO = _lEN %% rATE8 /\ iTERS = _lEN%/rATE8 /\
        ...
        + auto . 
          ...
          rewrite /valid /is_init /= => &m /> ? h *.  have:= inbounds_size => /= ?.
          have h1: (_offset + i{m} * rATE8{m} + rATE8{m} = _offset + (i{m} + 1) * rATE8{m}). smt().
          split. smt( BArray224.init_arrP).
          move=> *. rewrite h1. split. smt().
          move => *. split. move => /> *.
          + have ?: (_lEN%/rATE8{m} <= _lEN). case(rATE8{m}< _lEN).  smt(). case (rATE8{m} = _lEN).  smt().  case(_lEN < rATE8{m}). move => *. have ? : _lEN%/rATE8{m} = 0. print divz_eq0. apply  divz_eq0. smt().  smt(). smt(). smt().  smt(all_cat).
          smt(and_iota).
        auto .
        rewrite /valid /is_init /= => &m /> *.  split; smt(and_iota all_cat).
...
proof. 
  ...
  + sp.
    ...
    + if.
      + auto.
        ...
        auto. rewrite /is_init /valid /= => &m /> *. split. move => *. split.  smt(). move => *. split.  smt().  move => *. split. smt(). move => *. split. smt().  move => *. rewrite !all_cat /=. smt(). move => *. split.  smt(). move => *. split.  smt().  move => *. split. smt(). move => *. split. smt().  move => *. rewrite !all_cat /=. smt().
      if. auto. rewrite /is_init /valid /= => &m /> *. split. rewrite !all_cat /=. smt(). smt().
      auto.
      ...
      auto.  rewrite /is_init /valid /= => &m /> *. split. smt(). move => *. split. smt().  move => *. split.  smt(). move => *. split.  smt().  move => *. split. rewrite !all_cat /=. smt(). smt().
    auto. smt(all_cat).   
  ...
  + if. auto. 
    + while (valid trace___addstate_array_avx2x4 /\ 0<=_offset /\ 0<=lEN /\ 0<=at /\
           ...
           _offset <= offset /\ lEN <= _lEN).
      + auto. rewrite /is_init /valid /= => &m /> *. split. rewrite !all_cat /=. smt(). smt(). auto.
      while (valid trace___addstate_array_avx2x4 /\ 0<=_offset /\ 0<=lEN /\ 0<=at /\
             lEN + offset - ((at - 4 * (aT %/ 8))*2)  = _lEN + _offset /\
           _lEN + _offset <=size /\ aT + lEN = _aT + _lEN /\ _aT + _lEN < 200 /\ 0<=_aT /\ 
      ...
      + auto. ecall(__4u64x4_u256x4_trace param_22 param_21 param_20 param_19). auto.
        rewrite /is_init /valid /= => &m /> *. split. rewrite !all_cat /=. smt(). smt().   
        auto. rewrite /is_init /valid /= => &m /> *.
        split. split. rewrite !all_cat /=. smt(). smt().
        smt(all_cat).
    auto.  rewrite /is_init /valid /= => &m /> *. smt().
  if.
  + auto. ecall (__aread_subu64_trace param_3 b_param param_2 param_1 param_0 param).
    ...
    auto . rewrite /is_init /valid /= => &m /> *. 
    split. smt(BArray800.init_arrP). move => *.
    split; rewrite !all_cat /=;  smt(BArray800.init_arrP).
  auto. rewrite /is_init /valid /= => &m /> *.
  smt(all_cat BArray800.init_arrP).
qed .
...
proof.
  ...
  + auto.
    ...
    + auto. ecall (_keccakf1600_avx2x4_trace param_21 (BArray800.init_arr (W8.of_int 255) 800)).
      ...
      rewrite /valid /is_init /= => &m /> *.
      have ?: (rATE8{m} <= (lEN{m} %/ rATE8{m} - i{m}) * rATE8{m} ). smt().
      split. smt(BArray800.init_arrP). move => *. rewrite !all_cat /=. split. smt().  split. smt().  split. smt().  split. smt().  split. smt().  split. smt().  split. smt(). smt().
    if. 
...
have /= ? := modzMDr (-1) (lEN{m} + aT{m}) (rATE8{m}). smt(). smt(). 
      smt(all_cat).
    auto. move => *. split. smt(all_cat).  move => *.  smt(all_cat).          
  seq 20: ( 0 <= offset /\ offset <= size /\ valid trace___absorb_array_avx2x4 /\
              0<=aT /\ aT<=200 /\ 0 < rATE8 /\ rATE8 < 200).
  + auto.
...
proof.
  ...
  + while (valid trace___dumpstate_array_avx2x4 /\ 0<=i /\ i<= 32*(lEN %/ 32) /\0<=_offset /\
    ...
    + auto. ecall (__4u64x4_u256x4_trace param_22 param_21 param_20 param_19).
      auto. rewrite /is_init /valid /fold_len /= => &m /> *. split. rewrite !all_cat. smt(). smt().
    auto. smt(all_cat and_iota).
  ...
  + auto. ecall(__awrite_subu64_trace param_3 b_param param_2 param_1 param_0 param).
    ...
    auto .  rewrite /is_init /valid /fold_len /= => &m /> *.  split. smt().
    move => *. rewrite !all_cat /=. smt(and_iota).
  auto. smt(and_iota).
qed.
...
proof.
  ...
  + seq 1: ( valid trace___squeeze_array_avx2x4 /\ 0<=_offset /\ _offset + _lEN <=size /\
    ...
    + auto .
      ...
      + while ( valid trace___squeeze_array_avx2x4 /\ 0<=_offset /\ _offset + _lEN <=size /\
        ...
        + auto.
          ...
          split. smt(BArray800.init_arrP). move => *.
          split. smt(). move => *. rewrite !all_cat /=. split.  smt().  split. smt(). split. smt(and_iota).  split. smt(and_iota). split. smt(and_iota). split. smt(and_iota).  smt(and_iota). 
    ...
    + auto.
      ...
      rewrite /is_init /valid /fold_len /= => &m /> *. split. smt(BArray800.init_arrP). 
      move => *. split.  smt(). move => 10?. rewrite !and_iota !all_cat /= => *.
      smt().
    auto.
